 ...
 UpdateName = (e) => {
  ...
  res = this.objects.map(function(item) {
    ...
    if(item.name.indexOf(this.inputValue)>= 0)
    {
      ...
      return (
        <p key={item.id} className="border blockObject" style={{margin:"10px", fontSize: "28px"}}>
        {item.name}</p>
        );
    }
    else 
    ...
    if (this.page == "main"){
        ...
        <div className="center"> 
          <div className="mainPart">
            <div className = "blockUp" >
              <a className="btn-floating btn-large waves-effect waves-light blue leftBut" id="btnFilter" ><i className="large material-icons">build</i></a>
            ...
            <div className="hightBord fullScreen block gray hightSmoleBlock">
              <form className="col s12" id = "formFindName" method = "POST">
                  <input 
                    ...
                    type="text" className="input-field col s6 row validate" 
                    style={{margin: "0px 0px 0 10px"}}
                    />
                  <a 
                    onClick={ (e) => {this.UpdateName(e)} } className ="btn-floating btn-medium waves-effect waves-light orange rightButmedium" 
                    id = "btnFind" 

                  ><i className="material-icons">find_in_page</i></a>
              </form>
            </div>
      ...
      return(
        <div className="center"> 
          <div className="mainPart">
            <div className = "blockUp" >
              <a className="btn-floating btn-large waves-effect waves-light blue leftBut" id="btnFilter" ><i className="large material-icons">arrow_back</i></a>
              <h1 className = "center">Карточка Объекта</h1> 
              <a className ="btn-floating btn-large waves-effect waves-light yellow rightBut" id="btnEdit"><i className="large material-icons">create</i></a>
            </div>
